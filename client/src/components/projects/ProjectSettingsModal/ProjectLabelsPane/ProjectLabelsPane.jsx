...
const ProjectLabelsPane = React.memo(() => {
  ...
  if (step) {
    switch (step.type) {
      ...
      case StepTypes.EDIT: {
        const currentLabel = projectLabels.find((label) => label.id === step.params.id);

        if (currentLabel) {
          return <EditStep projectLabelId={currentLabel.id} onBack={handleBack} />;
        }
            ...
            {({ innerRef, droppableProps, placeholder }) => (
              ...
              >
                {filteredLabels.map((item, index) => (
                  ...
                  >
                    {(
                      { innerRef: innerDraggableRef, draggableProps, dragHandleProps },
                      { isDragging },
                    ) => {
                      const contentNode = (
                        <div
                        ...
                        >
                          ...
                          <Button
                            ...
                            disabled={!item.isPersisted}
                            className={styles.button}
                            onClick={() => dispatch(entryActions.deleteProjectLabel(item.id))}
                          />
                        </div>
                      );
...
const EditStep = React.memo(({ projectLabelId, onBack }) => {
  ...
  const projectLabel = useSelector((state) => selectProjectLabelById(state, projectLabelId));
