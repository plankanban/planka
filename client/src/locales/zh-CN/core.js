...
export default {
  ...
  translation: {
    common: {
      ...
      addNotificationService_title: '添加通知服务',
      ...
      authentication: '认证',
      autoLogout: '自动退出',
      autoLogout_10m: '10 分钟后',
      autoLogout_12h: '12 小时后',
      autoLogout_2m: '2 分钟后',
      autoLogout_30m: '30 分钟后',
      autoLogout_5m: '5 分钟后',
      autoLogout_never: '从不',
      autoLogoutWarning_body: '由于长时间无操作，您将在 {{seconds}} 秒后退出登录。',
      autoLogoutWarning_title: '您即将退出登录',
      background: '背景',
      ...
      color: '颜色',
      comments: '评论',
      confirmCodesSaved: '我已将这些代码保存在安全的地方。',
      ...
      proFeatureDashboard: '✨ 跨所有项目的统一仪表板',
      proFeatureExport: '✨ 将卡片导出为 PDF 用于汇报',
      proFeatureMobile: '✨ 在手机和平板上使用 PLANKA',
      proFeatureSso: '✨ 使用企业目录登录 (SSO)',
      proFeatureThemes: '✨ 五种主题与自定义强调色',
      proFeatureTimeline: '✨ 时间轴：一条轴线纵览全年',
      proTrialSelfHosted: '✨ 在您自己的服务器上试用 PLANKA Pro 30 天',
      dismissProBannerFor30Days: '关闭 30 天',
      customerPanel_title: '客户面板',
      dangerZone_title: '危险区域',
      disable2fa_title: '禁用双因素认证',
      disable2faWarning: '您需要确认当前密码和一个 TOTP 代码。现有会话将保持登录状态。',
      enable2fa_title: '启用双因素认证',
      enabledOn: '启用于 {{date}}',
      enterCodeFromApp: '请输入身份验证器应用中的 6 位验证码。',
      enterPasswordToContinue: '请确认您的当前密码以继续。',
      enterTotpOrRecoveryCode:
        '请输入身份验证器应用中的 6 位验证码，或其中一个恢复代码 (xxxxx-xxxxx)。',
      expires: '过期时间',
      invalidTotpCode: 'TOTP 或恢复代码无效',
      lastUsed: '最后使用',
      never: '从不',
      noTrustedDevices: '没有受信任的设备。',
      openBoardActions: '打开面板操作',
      openCardActions: '打开卡片操作',
      openListActions: '打开列表操作',
      openNotifications: '打开通知',
      openProjectActions: '打开项目操作',
      openProjectSettings: '打开项目设置',
      orEnterSecretManually: '或手动输入此密钥：',
      recoveryCodesExhausted:
        '所有恢复代码均已使用。请重新生成一组新代码，以便在丢失身份验证器应用时仍有备用方案。',
      recoveryCodesIntro:
        '请将这些代码保存在安全的地方——如果您无法访问身份验证器应用，每个代码可使用一次。',
      recoveryCodesLow_other: '仅剩 {{count}} 个恢复代码，请尽快重新生成。',
      regenerateRecoveryCodes_title: '重新生成恢复代码',
      regenerateRecoveryCodesIntro: '生成新代码将使所有现有的恢复代码失效。',
      reset2fa_title: '重置 2FA',
      reset2faWarning:
        '重置 2FA 将使该用户退出所有会话并移除其 TOTP 设置。该用户需要在下次登录时重新设置。',
      saveTheseCodes_title: '保存这些恢复代码',
      scanQrCodeWithApp: '请使用 Google Authenticator 或 Authy 等身份验证器应用扫描此二维码。',
      security_title: '安全',
      showHiddenProjects: '显示隐藏的项目',
      totpCode: 'TOTP 代码',
      totpOrRecoveryCode: 'TOTP 或恢复代码',
      trustedDevices_title: '受信任的浏览器',
      trustedDevicesHint:
        '被标记为受信任的浏览器在 30 天内将跳过第二重验证。请撤销您不再认识的浏览器。',
      twoFactor_enabled: '双因素认证已启用。',
      twoFactor_intro:
        '使用身份验证器应用生成的基于时间的一次性验证码，为您的账号增加一层额外的安全保护。',
    ...
    action: {
      ...
      emptyTrash_title: '清空回收站',
      enable2fa: '启用 2FA',
      enableEditMode: '进入编辑模式',
      filterByCurrentUser: '通过当前用户筛选',
      goBack: '返回',
      hideHiddenProjects: '不显示隐藏的项目',
      ...
      stop: '结束',
      subscribe: '关注',
      switchToGridProjectsView: '以网格显示项目',
      switchToGridView: '切换到网格视图',
      switchToGroupedProjectsView: '分组显示项目',
